...
const Callback: React.FC = () => {
    ...
    const processAuth = async () => {
      try {
        ...
        if (accessToken) {
          ...
          setStatus('Não conseguimos sentir sua energia. Vamos tentar novamente.');
          console.error('Callback page reached without access_token or error in hash.');
          setTimeout(() => navigate('/'), 4000);
        }
      } catch (e) {
        console.error('Falha ao processar o callback do Spotify:', e);
        setStatus('Houve uma interferência cósmica. Retornando ao ponto de partida.');
        setTimeout(() => navigate('/'), 4000);
      }
    };
  ...
  return (
    <Box $width={"100dvw"} $height={"100dvh"} $background={"#CDECCE"} $padding={"0px 30px"}>
      <GrainOverlay />
      <Box $width={"150px"} $height={"150px"} $borderRadius={"100px"} $overflow={"hidden"}>
        <AuraBlobs
      ...
      <Box $width={"100%"} $height={"15%"} $justifyContent={"flex-start"} $padding={"15px 0px"}>
        <BlurText
          key={status}
          text={status}
          delay={100}
